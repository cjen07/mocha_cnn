...
function classify{T<:AbstractString}(classifier::ImageClassifier, filenames::Vector{T})
  return classify(classifier, map(Images.load, filenames))
end

#-- Classify a single image
function classify(classifier::ImageClassifier, image::Array{ColorTypes.RGB{FixedPointNumbers.Normed{UInt8,8}},2})
  results = classify(classifier, ImageMeta[image])
  return results[1]
end

#-- Classify a bunch of images
function classify(classifier::ImageClassifier, images::Vector{ImageMeta})
...
################################################################################
#-- Propress images
function preprocess(classifier::ImageClassifier, images::Vector{ImageMeta})
  map(images) do image
    ...
    if classifier.grayscale
      if colorspace(image) != "Gray"
        image = convert(ImageMeta{Gray}, image)
      end
    else
      image = convert(ImageMeta{RGB}, image)
      image = permuteddimsview(channelview(image), (2,3,1)) # separate color channels
    end

    data = convert(Array, image)
    data = reshape(data, size(data,1),size(data,2),size(data,3))
    if coords_spatial(image) == (2,1,3)
      # row major
    elseif coords_spatial(image) == (1,2,3)
      data = permutedims(data, (2,1,3)) # permute to row-major
    else
      error("Unknown spatialorder: $(coords_spatial(image))")
    end
...
function classify_batch(classifier::ImageClassifier, images::Vector{ImageMeta}, idx::Int)
  #-- prepare the data
